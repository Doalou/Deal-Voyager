...
export const symaMobileScrapeLogic: ScraperConfig['scrapeFunction'] = async (page) => {
    try {
        ...
        const plans = await page.evaluate(() => {
            ...
            const items = document.querySelectorAll('.forfaits_top_list_item, .forfait-card, [class*="offre_"]');

            items.forEach(item => {
                const idProduct = item.getAttribute('data-id_product') || item.getAttribute('data-product-id') || Math.random().toString();
                ...
                if (priceAttr && parseFloat(priceAttr) > 0) {
                    ...
                    if (priceMainEl && priceCentsEl) {
                        const rawPrice = priceMainEl.textContent!.trim() + '.' + priceCentsEl.textContent!.replace(/[^\d]/g, '');
                ...
                const has5gClass = item.classList.contains('_is_5G') || item.innerHTML.toLowerCase().includes('5g');
                if (genEl || has5gClass) {
                    p.is5G = true;
                }

                // Frais d'activation
                const fraisAmountEl = item.querySelector('#frais_amount, .frais_amount');
                if (fraisAmountEl && fraisAmountEl.textContent) {
                    p.activationPrice = parseFloat(fraisAmountEl.textContent.replace(/[^\d,.]/g, '').replace(',', '.'));
                }

                // EU Data from detail item
                const detailEl = document.querySelector(`.forfait_detail_element_wrapper[data-id_product="${idProduct}"]`);
                if (detailEl) {
            ...
            const finalPlans = Array.from(planMap.values()).filter(p => p.price > 0);
            ...
            const actMatch = bodyText.match(/frais\s*d['\u2019]?\s*activation\s*(?::|\u00e0)?\s*(\d+(?:[,.]\d{1,2})?)\s*€/i);
            if (actMatch) globalActivation = parseFloat(actMatch[1].replace(',', '.'));
            ...
            if (/sim\s*gratuit/i.test(bodyText) || /sim\s*offert/i.test(bodyText)) {
                ...
                for (const pat of sps) {
                    ...
                    if (m) { globalSim = parseFloat(m[1].replace(',', '.')); break; }
                }
            }
            ...
            return finalPlans.map((p: any) => ({
                ...
                simPrice: null,
                activationPrice: null,
                cancellationPrice: null, // by default sans engagement
            }));
        });
        ...
        console.log(`[Syma Mobile] Plans trouvés et validés : ${validPlans.length}`);
