            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                for (let j = i + 1; j < Math.min(lines.length, i + 25); j++) {
                    ...
                    if (afterPromo) {
                        ...
                        // Important: stop at first regular price to avoid "Prix client Box" below
                        break;
                    }
                ...
                if (!results.some(r => r.dataGb === dataGb && r.price === finalPrice)) {
                    results.push({ planName, dataGb, price: finalPrice, calls, networkGeneration: gen, dataEuGb: euGb, simPrice, activationPrice, cancellationPrice });
                }
            }
